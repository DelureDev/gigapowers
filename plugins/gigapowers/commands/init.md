...
Invoke `codex:setup` via the Skill tool with `--enable-review-gate` as args to
enable the stop-time review gate, then **verify it actually took** — `codex:setup`
reports the review-gate state; confirm it reads **enabled** (look for
...
(`codex:setup` is *not* on the user-only `/codex:*` list — see
[orchestrating-codex](../skills/orchestrating-codex/SKILL.md) — so Claude can
run it directly.)
